...
func (w *writer) writeAllObjectsOld(acc *Accessor, parentLevel bool) {
...
func (w *writer) writeHeadersIfNeeded(headers []string, headerRowFieldTypes []string) {
	if len(headers) == 0 {
		return
	}
	...
	//w.buffer.writeString("<")
	w.writeTabularHeaderObject(headers, wasHeaderString, headerRowFieldTypes)
